<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Tambah Transaksi Barang Keluar Baru') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('admin.outbound-transactions.store') }}" method="POST" class="space-y-4">
                        @csrf
                        ...
                        <div>
                            <label for="quantity" class="block text-sm font-medium text-gray-700">Jumlah Barang Keluar</label>
                            ...
                            @error('quantity')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                            <p class="text-xs text-gray-500 mt-1">Pastikan jumlah keluar tidak melebihi stok yang tersedia.</p>
                        </div>

                        <div>
                            <label for="recipient" class="block text-sm font-medium text-gray-700">Penerima Barang (Opsional)</label>
                            <input type="text" name="recipient" id="recipient" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" value="{{ old('recipient') }}">
                            @error('recipient')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label for="purpose" class="block text-sm font-medium text-gray-700">Tujuan Barang Keluar</label>
                            <input type="text" name="purpose" id="purpose" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" value="{{ old('purpose') }}" required>
                            @error('purpose')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
